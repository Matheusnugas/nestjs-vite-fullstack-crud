describe('Tasks Management Flow', () => {
  ...
  it('should create a new task successfully', () => {
    ...
    cy.get('.fixed.inset-0').last().within(() => {
      cy.get('input[placeholder="Task title"]').first().type('Test Task Title', { force: true });
      cy.get('textarea[placeholder="Task description (required)"]').first().type('This is a test task description', { force: true });
  ...
  it('should create a task with title and description (both required)', () => {
    ...
    cy.get('.fixed.inset-0').last().within(() => {
      cy.get('input[placeholder="Task title"]').first().type('Task with title and description', { force: true });
      cy.get('textarea[placeholder="Task description (required)"]').first().type('This is a required description', { force: true });
      cy.get('button[type="submit"]').first().click({ force: true });
    });
  ...
  it('should show validation error when only title is provided', () => {
    ...
    cy.get('.fixed.inset-0').last().within(() => {
      cy.get('input[placeholder="Task title"]').first().type('Task with only title', { force: true });
      cy.get('button[type="submit"]').first().click({ force: true });
    });
  ...
  it('should show validation errors for empty form', () => {
    ...
    cy.contains('Title and description cannot both be empty.', { timeout: 10000 }).should('exist');
  });
  ...
  it('should mark task as completed', () => {
    ...
    cy.waitForModal();
    cy.get('.fixed.inset-0').last().within(() => {
      cy.get('input[placeholder="Task title"]').first().type('Task to complete', { force: true });
      cy.get('textarea[placeholder="Task description (required)"]').first().type('This task will be marked as completed', { force: true });
      cy.get('button[type="submit"]').first().click({ force: true });
    });
    cy.contains('Task created successfully!').should('be.visible');

    cy.contains('Task to complete').closest('li').within(() => {
  ...
  it('should mark completed task as pending', () => {
    ...
    cy.waitForModal();
    cy.get('.fixed.inset-0').last().within(() => {
      cy.get('input[placeholder="Task title"]').first().type('Task to toggle', { force: true });
      cy.get('textarea[placeholder="Task description (required)"]').first().type('This task will be toggled', { force: true });
      cy.get('button[type="submit"]').first().click({ force: true });
    });
    cy.contains('Task created successfully!').should('be.visible');

    cy.contains('Task to toggle').closest('li').within(() => {
      cy.get('button[title="Complete"]').click();
    });
    cy.contains('Task status updated!').should('be.visible');
  ...
  it('should edit an existing task', () => {
    ...
    cy.waitForModal();
    cy.get('.fixed.inset-0').last().within(() => {
      cy.get('input[placeholder="Task title"]').first().type('Original Title', { force: true });
      cy.get('textarea[placeholder="Task description (required)"]').first().type('Original description', { force: true });
      cy.get('button[type="submit"]').first().click({ force: true });
    });
    cy.contains('Task created successfully!').should('be.visible');

    cy.contains('Original Title').closest('li').within(() => {
      cy.get('button[title="Edit"]').click();
    });
    cy.waitForModal();

    cy.get('.fixed.inset-0').last().within(() => {
      cy.get('input[placeholder="Task title"]').first().clear({ force: true }).type('Updated Title', { force: true });
      cy.get('textarea[placeholder="Task description (required)"]').first().clear({ force: true }).type('Updated description', { force: true });
      cy.get('button[type="submit"]').first().click({ force: true });
    });
  ...
  it('should delete a task', () => {
    ...
    cy.waitForModal();
    cy.get('.fixed.inset-0').last().within(() => {
      cy.get('input[placeholder="Task title"]').first().type('Task to delete', { force: true });
      cy.get('textarea[placeholder="Task description (required)"]').first().type('This task will be deleted', { force: true });
      cy.get('button[type="submit"]').first().click({ force: true });
    });
    cy.contains('Task created successfully!').should('be.visible');

    cy.contains('Task to delete').closest('li').within(() => {
